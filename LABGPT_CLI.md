...
- `--device {cpu,cuda,auto}`: Device for embedding models (default: `auto`)
  - `auto`: Automatically selects GPU if available and not in use by other tasks
  - `cuda`: Forces GPU usage (may conflict with training tasks)
  - `cpu`: Uses CPU (slower but no GPU conflicts)
...
# Use CPU to avoid GPU conflicts during training
python labgpt_cli.py rag \
  --papers /path/to/papers \
  --index ./my-rag-index \
  --device cpu
